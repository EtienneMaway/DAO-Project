...
contract ETMW is  AccessControl {
    ...
    modifier OnlyStackholder(string memory message) {
    ...
    function createProposal(
     ...
     OnlyStackholder("Proposal Creation Allowed for Stakeholders only")
    {
    ...
    function performVote(uint256 proposalId, bool choosen)
        external
        OnlyStackholder("Unauthorized: Stakeholders only")
    {
        ProposalStruct storage proposal = raisedProposals[proposalId];
    ...
    function contribute() payable external {
        require(msg.value > 0 ether, "Contribute more than 0 ether, please");
    ...
    function getStakeholderVotes()
        external
        view
        OnlyStackholder("Unauthorized: not a stakeholder")
    ...
    function isStakeholder() external view returns (bool) {
        return stakeholders[msg.sender] >= MIN_STAKEHOLDER_CONTRIBUTION;
    }
